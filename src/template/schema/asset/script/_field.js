...
$(window).on('field-form-submit', function(e, target) {
  ...
  var schema = $('form.schema-form input.schema-name').val();

  $(window).trigger('panel-form-submit', [target, function(response) {
    var row = $(response);
    var newname = row.data('name');
    $('span.field-name', row).text(`${schema}_${newname}`);

    var original = $(`#field-${name}`);
    if (original.length) {
      original.replaceWith(row);
    } else {
      $('form.schema-form table.table-fields').append(row);
    }

    $('form.schema-form div.alert-table-fields-empty').addClass('d-none');
...
$(window).on('field-remove-click', function(e, target) {
  ...
  if (!body.children().length) {
    $('form.schema-form div.alert-table-fields-empty').removeClass('d-none');
  }
});
